        ...
        if image_atual:
            ...
            for w in range(width):
                for h in range(height):
                    ...
                    if g > 119 and r < 118 and b < 200:
                        pixels[w, h] = (0, 0, 0, 0)
            show_image()
        else:
            sg.popup("Nenhuma imagem aberta.")
    except Exception as e:
        sg.popup(f"Erro ao substituir o fundo verde: {str(e)}")

def recrop_image(x, y, width, height):
    global image_atual
    global previous_state
    try:
        if image_atual:
            previous_state = image_atual.copy()
            image_atual = image_atual.crop((x, y, width, height))
            show_image()
        else:
            sg.popup("Nenhuma imagem aberta.")
    except Exception as e:
...
def draw_rectangle(x, y, width, height):
    ...
    try:
        ...
        draw.rectangle((x, y, width, height), outline="red")
        ...
        sg.popup(f"Erro ao desenhar o retângulo: {str(e)}")
...
layout = [
    [sg.Menu([
            ...
            ['Imagem', [
                ...
                'Filtro', ['Preto e Branco', 'Sépia', 'Negativo', '4 bits', 
                           ...
                           'Filtro mínimo', 'Filtro máximo', 'Remover fundo verde'],
                'Cortar', ['Cortar']
            ]],
    ...
    [sg.Image(key='-IMAGE-', size=(800, 600), background_color='black')],
]

window = sg.Window('Photo Shoping', layout, finalize=True, background_color='black', font="Arial")
...
while True:
    ...
    if event in (sg.WINDOW_CLOSED, 'Fechar'):
        ...
        draw_rectangle(10, 25, 6, 5)
    elif event == 'Remover fundo verde':
        replace_green_background()
    elif event == 'Desenvolvedor':
        sg.popup('Desenvolvido por Fabiana Sayuri - BCC 6º Semestre')
